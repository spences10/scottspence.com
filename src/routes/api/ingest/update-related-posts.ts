...
export async function update_related_posts_table() {
  ...
  try {
    // Get all post IDs
    const all_posts_result = await client.execute('SELECT post_id FROM post_embeddings')
    const all_post_ids = all_posts_result.rows.map((row: any) => row.post_id)
    ...
    for (const post_id of all_post_ids) {
      try {
        ...
        // Get related posts
        const related_posts = await get_related_posts(post_id, 5)
        console.log(`Found ${related_posts.length} related posts for ${post_id}`)
