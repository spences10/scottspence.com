...
describe('normalise_referrer', () => {
	it('groups Google variants', () => {
		expect(normalise_referrer('https://www.google.com/')).toBe(
			'Google',
		);
		expect(normalise_referrer('https://www.google.com.hk/')).toBe(
			'Google',
		);
		expect(normalise_referrer('https://www.google.co.uk/')).toBe(
			'Google',
		);
		expect(normalise_referrer('https://www.google.fr/')).toBe(
			'Google',
		);
		expect(
			normalise_referrer('https://www.google.com/search?q=sveltekit'),
		).toBe('Google');
		expect(
			normalise_referrer(
	...
	it('returns hostname for AI assistants (no grouping needed)', () => {
		expect(normalise_referrer('https://chatgpt.com/')).toBe(
			'chatgpt.com',
		);
	...
	it('returns null for internal domains', () => {
		...
		expect(normalise_referrer('https://www.spences10.dev/')).toBe(
			null,
		);
	...
	it('returns cleaned hostname for unknown sources', () => {
		...
		expect(normalise_referrer('https://motion.dev/')).toBe(
			'motion.dev',
		);
	});
});
...
describe('is_internal_referrer', () => {
	it('identifies internal domains', () => {
		expect(is_internal_referrer('https://scottspence.com/')).toBe(
			true,
		);
...
describe('aggregate_referrers', () => {
	...
	it('filters out internal domains', () => {
		const raw = [
			...
			{
				referrer: 'https://scottspence.com/',
				views: 50,
				visitors: 30,
			},
		];
