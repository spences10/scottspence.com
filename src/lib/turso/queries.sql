...
-- Modify as needed based on what you want to order by
-- stats
WITH
  TotalPosts AS (
    SELECT
      'Total Posts (' || strftime ('%Y', date) || ')' AS stat,
      COUNT(*) AS value,
      'Total' AS extra1,
      (
        SELECT
  ...
  ShortestReadingTime AS (
    SELECT
      'Shortest Reading Time (minutes)' AS stat,
      MIN(reading_time_minutes) AS value,
      '' AS extra1,
      '' AS extra2
    FROM
      posts
  ),
  LongestReadingTime AS (
    SELECT
      'Longest Reading Time (minutes)' AS stat,
      MAX(reading_time_minutes) AS value,
      '' AS extra1,
      '' AS extra2
    FROM
      posts
  ...
  PopularPosts AS (
    ...
    ORDER BY
      pp.pageviews DESC
    LIMIT
      10 -- Adjust as needed; each row will be a separate entry in the union
  ),
  TotalReactions AS (
    SELECT
