<script lang="ts">
	...
	const sorted_leaderboard = $derived(
		[...leaderboard].sort((a, b) => {
			...
			return (
				order * ((b[sort_by] as number) - (a[sort_by] as number))
			);
		}),
	);
...
<section class="m-0 mb-20 sm:-mx-30 lg:-mx-40">
	<div class="mb-4 flex items-center justify-end">
		<select
			class="select-bordered select mr-2 select-sm"
			bind:value={sort_by}
			onchange={(e) =>
				handle_sort_change(
			...
			>
				...
				>
					...
					<div class="mt-5 flex flex-wrap justify-between">
						{#each reactions as reaction}
							<span
								class="btn mr-2 mb-2 min-w-[calc(50%-0.5rem)] flex-1 text-xl btn-primary md:min-w-0 md:flex-none"
							>
								{reaction.emoji}
								{page[reaction.type]}
							</span>
