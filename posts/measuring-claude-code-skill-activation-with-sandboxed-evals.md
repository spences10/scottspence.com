...
## WTF is a forced eval?

Tl;Dr, you use `UserPromptSubmit` in a hook to prompt inject Claude to
say "hold on, check each skill first, decide YES or NO, _then_ act."
It's a commitment mechanism. Claude has to explicitly evaluate whether
each skill matches the query before proceeding.

Essentially add a hook:

Create `.claude/hooks/skill-forced-eval-hook.sh` in your project (or
globally in `~/.claude/hooks/`)
